...
- **Geri yükleme sürümlenmemiş işi silebiliyordu.** Sürüm geçmişinden bir dosyayı geri yüklemek eski sürümü kaydedilmiş ama sürümlenmemiş değişikliklerin üstüne doğrudan yazıyordu: sonra o değişiklikler ne diskte ne Ctrl+Z'de ne geçmişte ne de kurtarma klasöründe vardı, uyarı ise yalnız kaydedilmemiş değişikliklerden söz ediyordu. Şimdiki hâl artık önce "Geri yüklemeden önce: <dosya>" adıyla sürüme alınıyor (kaydedilmemiş değişiklik de önce kaydedilip ona giriyor); o sürüm alınamazsa hiçbir şey geri yüklenmiyor. Ctrl+K artık proje klasörü dışındaki sekmeleri diske yazmıyor, fark ekranı da diskteki dosyayla değil editördeki metinle karşılaştırıyor
- **Otomatik kaydetme "Kaydetme" cevabını boşa çıkarabiliyordu.** Otomatik kaydetme (öntanımlı açık, üç dakikada bir) "Değişiklikler kaydedilsin mi?" sorusu ekrandayken de çalışıyordu; atmayı seçtiğiniz metin yine diske gidebiliyordu. Artık herhangi bir soru açıkken bekliyor. Çökme kurtarmada "Geri Yükle"den hemen sonra da diskte kurtarılabilir hiçbir şeyin olmadığı bir an vardı; geri yüklenen sekmeler anlık görüntülerini artık temiz kapanışa kadar tutuyor
- **Kapanış ayracını yazıp Enter'a basmak onu silebiliyordu.** `\input{bolumler/giris}`, `\ref{fig:a}` ya da `\cite{knuth84}` harf harf yazılıp satır sonunda Enter'a basılınca bir önceki harfin açık bıraktığı tamamlama listesi kabul ediliyor, `}` siliniyor ve Enter yutuluyordu. `\begin{itemize}` ardından Enter ya da Tab gövde satırını da siliyordu, `\begin{figure}` ise `\begin{figure*` oluyordu. Görünür, gerçek bir pencerede ölçüldü
- **Tam yazılmış komuttan sonra Enter komutu değiştiriyordu.** O liste açık kaldığı için `\centering` ya da `\hline` ardından Enter yeni satır açmıyordu (bilinen 396 komutun 372'si böyle), `\item` madde imini kaldıran `\item[]` oluyordu, `\par` ise `\paragraph{}`. Liste artık tam bir komut yazılınca kapanıyor; Ctrl+Space uzun adayları yine gösteriyor. Yalnız `\begin{...}` içeren bir satırın hemen altında tamamlamayı Enter'la kabul etmek de imleci satırın sonuna atıyordu
- **Otomatik ayraçlar belgeyi bozabiliyordu.** `\$` yazmak matematiği açan ikinci bir `$` ekliyordu ("Missing $ inserted"). `%` yorumunun içinde `\begin{...}` yazmak yorumun dışına çalışan bir `\end{...}` koyuyordu. Az önce yazılan ayracı ya da `$`ı silmek eşini geride bırakıyordu; ortamın adı `}` dahil silinip yeniden yazılınca eklenen ikinci `\end` da `\item`leri listenin dışına itiyordu. CRLF dosyalarda (paketle gelen 132 şablonun 17'si) `\begin{...}` satırında Enter'dan sonraki tuş alt satıra düşüyordu. Her durum gerçek pdflatex ile sınandı: niyet edilen metin derleniyor, editörün ürettiği derlenmiyordu
- **Tamamlama, LaTeX'in kabul ettiği yazımlarda açılmıyordu.** Türkçe harfli yol ve etiketler (`\input{bölümler/`, `\ref{şekil`), virgülden sonra boşluklu `\cite{a, b}`, isteğe bağlı argümanlı `\cite[s.~5]{...}` ve `\cref{a,b}` listesinin ikinci anahtarı liste almıyordu. Yorumun içinde kapalı olması gereken tamamlama da yazarken açılıyordu
- **Bölüm dosyaları bütün bir belge sanılıyordu.** Bölümlerinde `% !TEX root` satırı olmayan bir tezde (paketle gelen bölüm dosyalarının hiçbirinde yok) F5 ve Ctrl+S bölümün derlenemeyeceğini söylüyordu; bölüme yapıştırılan görsel derlemenin bulamadığı bir yol alıyordu; F2 ile yeniden adlandırma öteki bölümlere dokunmuyordu; DOI ile ekleme, Kaynakça sekmesi, yazım dili ve `\input` ağacı yalnız bölüme bakıyordu. Bölüm sekmesinden dışa aktarma yalnız o bölümü aktarıyor, `\graphicspath` görsellerini gömmüyordu. Derleme motoru derlenen belgeden değil öndeki sekmeden geliyordu; "Birlikte Aç"a verilen göreli bir yol da çalışan pencerede başka bir projenin dosyasını açabiliyordu
- **Günlük dosyası kilitliyken uygulama açılmıyordu.** Başka bir program `latex-editor.log` dosyasını paylaşıma kapalı açık tutuyorsa (Word ve Excel böyle yapıyor) uygulama ilk penceresinden önce düşüyor ve hiçbir şey göstermiyordu. Artık o durumda dosyaya günlük yazılmıyor, uygulama açılıyor
- **Linux ve macOS'ta Durdur TeX'i durdurmuyordu.** Derlemeyi durdurmak ya da zaman aşımı yalnız TeX'i saran betiği öldürüyor, her iptal bir işlemci çekirdeğini %100'de tutan bir TeX süreci bırakıyordu. Artık bütün süreç grubu sonlandırılıyor. XeLaTeX ya da LuaLaTeX isteyen belge (fontspec) motor önerisini yeniden alıyor, TeX'in iki satıra böldüğü bir Unicode hatası da ipucunu gösteriyor
- **Daha açık hata paneli.** Hata satırları artık hangi dosyadan geldiğini söylüyor; bir paketin içinde doğan hata, paketin kendi dosyasındaki bir satırı değil sizin `\usepackage` satırınızı gösteriyor. Kendi makronuzun içinde kullanılan tanımsız bir komut (`\newcommand{\R}{\mathbb{R}}`) artık ipucunun makronun kendisini suçlamasına yol açmıyor
- **UTF-16 dosyalar ikili diye reddediliyordu.** Windows PowerShell'in `>` yönlendirmesiyle yazılmış bir `.tex` (örneğin `latexdiff eski.tex yeni.tex > fark.tex`) UTF-16 oluyor ve açılmıyordu. Artık UTF-8'e çevrilerek açılıyor; uyarı da ilk kaydın, otomatik kaydetme dahil, dosyayı UTF-8 yazacağını söylüyor
- **Referans denetimi adres ve koddaki `%`yi yanlış okuyordu.** `\url{...}`, `\href{...}` ya da `\verb|...|` içindeki `%` (adreslerde `%C3%87` gibi yüzde kodlu Türkçe harfler sık) yorum sanılıyordu; aynı satırdaki sonraki `\label` görünmüyor, ona giden her `\ref` tanımsız diye bildiriliyordu
- **Küçük düzeltmeler.** `\begin`/`\end` vurgusu yorumdaki ve sözel ortamdaki etiketleri saymıyor, aynı satırda iki kez geçen etikette doğru eşi buluyor, eski Mac satır sonlu dosyada da doğru satıra iniyor. PDF yer imleri sayfa başına değil başlığa gidiyor, kapattığınız düğümler derlemeler arasında kapalı kalıyor. Ayarlar penceresi kayıtlı değerleri artık kırpmıyor, büyük yazıda satır numaraları kesilmiyor. `\input` ağacı ve dil etiketi tema değişikliğini izliyor, son sekme kapanınca yan paneller boşalıyor. Güncelleme penceresi sürüm notunda `&amp;` göstermiyor ve yeniden açılışlar arasında günde en çok bir kez denetliyor
- **4302 test** (v1.0.30'da 4225)

### v1.0.30: Geride Kalan İkiz

- **Bozuk bir PDF sayfası uygulamayı hiçbir uyarı vermeden kapatabiliyordu.** Farenin sayfanın üzerinde gezinmesi, SyncTeX için Ctrl+tık ya da metin seçilirken derlemenin bitmesi Qt olay işleyicisinin içinde bir istisna doğuruyordu ve PyQt6 buna süreci durdurarak karşılık veriyor (Windows'ta çıkış kodu `0xC0000409`): ne diyalog ne günlük ne yığın izi, kaydedilmemiş her şey gidiyordu. Görüntüleyicideki kardeş yolların hepsi (yakınlaştırma, sunum, yer imleri, arama vurgusu, çift tık) korumayı çoktan almıştı; bu üçü o taramanın dışında kalmıştı
- **Tümünü Değiştir belgeyi sessizce bozabiliyordu.** `İ` harfini ayrıştırılmış biçimde taşıyan metin (`I` ve ardından birleşen nokta U+0307; macOS dosya adlarını böyle yazıyor, bazı PDF ve web sayfalarından kopyalanan metin de böyle geliyor) katlanınca kısalıyor ve ondan sonraki her eşleşme bir karakter kayıyordu: Bul yanlış metni seçiyor, Tümünü Değiştir `Burada şekil var.` satırını `BuradaŞEKİLl var.` yapıyordu. Klasörde Ara ve PDF arama vurgusu da aynı ofsetleri kullanıyordu. Eşleşmeler artık özgün metne geri eşleniyor; katlanınca boşalan bir arama da artık sonsuz döngüye girmiyor
- **Başlığında çapraz başvuru olan şekil dışa aktarımda bozuluyordu.** pandoc böyle bir başlığın içine bağlantı koyuyor (`bkz. Tablo [1](#tbl1)`) ve görsel deseni o bağlantıda duruyordu: belge içi `#tbl1` çapası mutlak dosya yoluna çevriliyor, başvuru kırılıyor ve yerel kullanıcı klasörünüz paylaşılan dosyaya sızıyordu; şeklin gerçek görsel yolu ise göreli kalıyordu. 39 şablonun tamamında uçtan uca sınandı: önce bir kırık bağlantı ve bir sızan yol, sonra hiçbiri
- **PDF görüntüleyicide sayfaya gitmek kendini geri alıyordu.** SyncTeX atlaması, yer imi ya da belge içi bağlantıdan sonra sayfa sayacı 100 ms içinde önceki sayfaya dönüyordu; küçük yakınlaştırmada sayfa düğmeleri hiçbir şey yapmıyordu, çünkü Qt `ensureWidgetVisible` çağrısını hedef zaten görünürken atlıyor. Arama işçisi de ilk açılışı derleyicinin PDF'i yeniden yazdığı ana denk gelirse o belge boyunca ölü kalıyordu; ikizi olan çizim işçisi bu dersi iki hafta önce almıştı
- **Bul sayacı bir önceki aramayı gösteriyordu.** Bul kutusu boşaltılınca bekleyen sayım durdurulmuyor, 300 ms sonra boş kutunun altına eski sorgunun sayısını yazıyordu
- **`\pageref{LastPage}` tanımsız referans sayılıyordu.** `lastpage` ve `totpages` bu etiketleri kendileri tanımlıyor; referans denetimi düzeltemeyeceğiniz bir hata gösteriyordu. Artık tanımlı sayılıyorlar, ama yalnız paket gerçekten yüklüyse
- **`\href`in bağlantı metni yazım denetiminden hiç geçmiyordu.** İki argüman da atlanıyordu, oysa ikincisi PDF'e basılıyor. 17 şablonda daha önce hiç denetlenmemiş 58 gerçek kelime vardı
- **Proje kökündeki şekil PDF'leri dosya ağacında gizleniyordu.** "Kökteki PDF derleme çıktısıdır" kuralı dosya adına bakılarak yapılmış bir sayıma dayanıyordu; belgenin dosyayı gerçekten çağırıp çağırmadığına bakılınca 39 şablonun 5'inde, 142 PDF'ten 6'sı yazarın kendi şekliydi. Aynı hata alt klasörler için zaten düzeltilmişti
- **`\includegraphics{` önerileri derlenmiş belgeleri şekil diye sunuyordu.** Süzgeç yalnız kaynağıyla birebir aynı adı taşıyan çıktıyı tanıyordu. Dosya adıyla değil sayfa sayısıyla ölçüldü: önerilen 84 PDF'in 51'i `main_pdflatex.pdf` gibi çok sayfalı belgelerdi. Adı yanındaki tam bir belgenin adını uzatan PDF (`main.tex` ve `main_pdflatex.pdf`) artık çıktı sayılıyor; şablonların gerçekten kullandığı 23 şeklin hiçbiri listeden düşmedi
- **`\input` edilen dosyadaki bir uyarıya tıklamak ana belgeye götürüyordu.** Hatalar dosyalarını `-file-line-error` açıldığından beri taşıyor; TeX'te uyarılar için öyle bir anahtar yok ve derleme betiği uyarı satırlarını çevrelerindeki dosya işaretleri olmadan iletiyordu, yani bölüm dosyasındaki her uyarı ana belgenin aynı numaralı satırını gösteriyordu. Betik artık TeX'in açtığı dosyaları izliyor ve böyle uyarıların başına kaynak dosyayı yazıyor. 39 şablonda 52 uyarı artık geldiği bölüm dosyasını gösteriyor; satırı gösterdiği dosyanın sonundan öte düşen uyarı sayısı 10'dan 4'e indi (kalan dördü paket dosyalarından geliyor)
- **Yeni: PDF belgenin istediğinden başka bir kâğıda çıktıysa Öneriler sekmesi bunu söylüyor.** A4 isteyen ama öntanımlısı US Letter olan bir TeX kurulumunda (macOS'ta olduğu gibi) derlenen belgeye, düzelten `geometry` seçeneğiyle birlikte tek satır düşüyor. Kâğıt sizin yerinize değiştirilmiyor, çünkü Letter isteyen şablonlar var (IEEE gibi). Ölçüldü: metin kaybolmuyor, değişen yalnız kâğıt boyu ve kenar boşlukları
- **4225 test** (v1.0.29'da 4181). Bu düzeltmelerin altısı aynı kalıbı izliyor: bir yerde alınmış ama ikizine hiç uygulanmamış bir ders
...
- **Artık paketi de söylüyor.** Tanımsız bir ortam ya da komut çoğu zaman bir `\usepackage` uzaklıktadır ve cevap tek kelimedir. Ölçülmüş iki harita eklendi: uygulamanın bildiği 79 ortamın 43'ü, kendi oluşturduğu boş belgede yok; tamamladığı 382 komut adının 44'ü orada tanımsız. Her satır gerçek derlemeyle iki yönde denendi ve bu, akla yatkın iki tahmini eledi: `amsthm` `theorem` ortamını tanımlamıyor (`\newtheorem` gerekiyor) ve `subfig` `subfigure` ORTAMINI vermiyor (`subcaption` veriyor). Dört komutun cevabı paket değil: `\chapter`, `\frontmatter`, `\mainmatter` ve `\backmatter` belge sınıfından geliyor ve `article` ile başlayan bir Türkçe tez tam bu duvara tosluyor
- **Eksik harf uyarısı zararı 35 kat küçük gösteriyordu.** XeLaTeX ya da LuaLaTeX ile `[T1]{fontenc}` birleşimi Türkçeye özgü dört harfi sessizce düşürüyor; derleme başarıyla bitiyor, harfler PDF'te yok. Uyarı kaç harfin düştüğünü sayıyor ama betik o satırları sayaca gelmeden tekilleştiriyordu, yani sayı "kaç farklı karakter" oluyordu. 140 düşen harf içeren bir belgede panel "4 karakter" diyordu. Artık 140 diyor ve kaç farklı harf olduğunu da yazıyor
- **Kaynakça sessizce çöküyordu.** bibtex biçem dosyasını ya da `.bib`i açamadığında, veya `.bib` sözdizimi bozuk olduğunda derleme yine başarıyla bitiyor ve kaynakça boş kalıyor. Betik bibtex çıktısını "error" ve "warning" kelimeleriyle süzüyordu; bibtex'in asıl tanısı ise bunların hiçbirini kullanmıyor: "I couldn't open style file", "I found no database files", "Illegal end of database file". Geriye kalan tek şey "(There were 2 error messages)" özeti oluyordu, mesajlar olmadan. Dört bozuk kurulumda ölçüldü; birinde panele tek satır bile ulaşmıyordu. Aynı süzgeç biber ve bibtex için iki ayrı yazımla duruyordu, artık tek
- **İngilizce belge Türkçe sözlükle denetleniyordu.** Yazım dili `babel` bildiriminden okunuyor ve `british` ile `en-GB`, uygulamanın ne sözlüğü ne de seçicide girdisi olan bir dile gidiyordu; sonuçta hiçbir şey seçilmiyor ve denetim o an seçili dille koşuyordu. babel'in kendi 28 İngilizce adıyla ölçüldü: 5'i doğru çözülüyor, 2'si boşluğa gidiyor, 21'i hiç tanınmıyordu (`ukenglish`, `canadian`, `australian`, `en-AU`...). Üç gerçek İngilizce şablonda panel 4642, 1944 ve 1333 bulgudan 95, 21 ve 34'e indi
- **Klasör adında boşluk varsa Markdown çıktısında görseller kayboluyordu.** CommonMark ve GitHub biçiminde bağ hedefinde boşluk olamaz, satır görsel yerine düz metin basılır; GitHub ve VS Code önizlemesi böyle davranıyor. pandoc'un kendi okuyucusu hoşgörülü olduğu için gözden kaçıyordu. Türkçe projeler her gün "Ölçüm Çalışması" gibi klasörlerde duruyor. Aynı turda öteki üç dışa aktarma biçimi de ölçüldü ve temiz çıktı
- **Dizin ve sözlük araçlarının süzgeci tersine çalışıyordu.** Sağlıklı koşuda içeriği yalnızca "0 warnings" olan sarı bir "makeindex uyarıları" bloğu çıkıyor, buna karşılık makeindex'in **reddettiği** (ve dizininizden düşen) bir girdi hiçbir iz bırakmıyordu: araç bunun için uçbirime hata satırı basmıyor ve sıfırla çıkıyor. Ölçüt artık sayının sıfır olmaması. Üç kol üç ayrı süzgeç kullanıyordu, artık tek
- **Ortam Denetimi kendine "derleme zincirinin tamamı" diyip üç aracı atlıyordu.** `bibtex`, `makeindex` ve `makeglossaries` derleme betiği tarafından çağrılıyor ama hiç sorulmuyordu; üçünün de eksikliği sessiz: başlık basılıyor, altı boş kalıyor. `makeglossaries` ayrıca tek başına eksik olabiliyor, çünkü motorlardan başka bir paketten geliyor. Denetim artık `makeglossaries-lite`ın da aynı işi gördüğünü biliyor; Perl'siz kurulumda olan tam budur
- **Her gün karşılaştığınız küçük şeyler.** Türkçe adlı bir görsel anlaşılmaz bir etiket üretiyordu (`Ölçüm Değerleri.png` → `fig:l-m-De-erleri`) ve F2, tablo sihirbazının kendi ürettiği etiketleri yeniden adlandırmayı reddediyordu; `İstanbul` yazınca Hızlı Dosya Aç dosyayı bulamıyordu; eski kodlamalı bir belge hiç kaydedilemiyordu; DOI ile çekilen başlıktaki `%` kaynakça girdisini sessizce kesiyordu; tezin ön maddesi anahat panelinde görünmüyordu; ve önsöz ayrı bir dosyadayken motor yanlış sinyallerden seçiliyordu
- **3923 test** (v1.0.25'te 3664). Bu tur tek bir sınıfa harcandı: doğru yazılmış ama gerçek boru hattıyla hiç beslenmeyen mekanizma, ve uygulamanın geri kalanının veremeyeceği bir şeyi vaat eden liste. Her kusur yazılmadan önce ölçüldü, yazıldıktan sonra yeniden ölçüldü, her birinin yanında bir gerileme testi var ve her test düzeltme bilerek bozularak düşerken görüldü. O denetimlerden biri, bir kabuk desenini Python'un düzenli ifade motoruyla okuyan bir kapıyı ortaya çıkardı

### v1.0.25: Uyarı Çıkmadan

- **PDF'te tıkladığınız yer.** Ters arama (PDF'te tıkla, kaynağa git) `synctex edit` birden çok kayıt döndürdüğünde sonuncusunu alıyordu; ileri yön aynı durumda ilkini alıyor ve gerekçesi kaynağında yazılı. 30 gerçek `.synctex.gz` üçlüsünden üretilen 143 noktada ölçüldü: 18 nokta çok kayıt döndürdü ve 18'inin 18'inde ilk ile son farklıydı. İstenen satırdan sapmanın ortalaması 245.1 satırdan 6.6'ya, en büyüğü 19114'ten 283'e indi; dosyada olmayan bir satıra gönderen nokta 2 idi, artık yok. 458 satırlık bir belgede son kayıt 19213. satırı gösteriyordu. Aynı taramada iki eksik daha çıktı: tıklanan nokta synctex'e tam sayıya kırpılarak veriliyordu (bir punto kayma), ve projeniz WSL'in kendi dosya sisteminde duruyorsa ters arama Windows'un açamayacağı bir yol döndürüyor, editörde hiçbir şey olmuyordu
- **Bayat SyncTeX verisi sessizce kullanılıyordu.** Ön koşul yalnız dosyanın varlığına bakıyordu; PDF'ten eski bir `.synctex.gz` varsa synctex eski yerleşime göre cevap veriyor ve hiçbir uyarı çıkmıyordu. Ulaşılabilir yol üretildi: `derle.sh` geçici dizinde derleyip `.gz`yi kopyalıyor, motor `.gz` üretmezse kopyalama sessizce atlanıyor ve eski dosya yerinde kalıyor. Belgenin kendi `\synctex=0` ataması bunu komut satırı bayrağına rağmen yapabiliyor. Eşik ölçümden geldi: 30 gerçek çiftte `.gz` ile PDF arasındaki fark en çok 1.4 saniye ve hiçbiri negatif değil; 2 saniyelik pay FAT/exFAT için, 30 çiftin 30'unda yanlış alarm yok
- **Kesilen bir yazma kaynakçanızı sıfırlayabiliyordu.** `.bib`e ekleme normalde dosyanın sonuna yazıyor, ama yeni girdi dosyanın kendi kodlamasına sığmazsa (DOI ile gelen kayıtlarda olağan) dosyanın tamamı yeniden yazılıyordu ve o yol `wb` kullanıyordu, yani açar açmaz boşaltıyordu. Hata enjeksiyonuyla üretildi: önce `.bib` 0 bayta düşüyor ve girdi gidiyordu, sonra 115 bayt ve girdi yerinde. Aynı turda kaynakça okuma yolu dış kehanetlere karşı ölçülüp temiz çıktı: 22 gerçek `.bib` `biber --tool` ile karşılaştırıldı, 306 anahtarın 306'sı aynı; gerçek `bibtex` ve `plain.bst` ile zorunlu alan uyarılarında fazladan uyarı 0, kaçırılan uyarı 0
- **Kelime sayısı düz yazı olmayanı sayıyordu.** Kehanet dışarıdan geldi: TeX Live'ın `texcount`u, 132 gerçek şablonda. Ortanca oran 1.06 ama kuyruk ağırdı, en uçtaki dosyada texcount'un 15 katı. Üç kaynak çıktı ve üçü de bu depoda başka bir yerde zaten biliniyordu: satır içi `\verb` (940 sahte kelime), çizim ortamları (717), `@` içeren komut adları (701). Toplamda 159839 kelimenin %1.5'i, ama dağılımı değil yoğunluğu önemli: tikz'li bir soru kâğıdında sayı ikiye katlanıyordu, 589 yerine 1224. Üçüncü bir kehanet, basılmış PDF, kimin haklı olduğunu söyledi: 57 belgenin 56'sında biz PDF'e texcount'tan daha yakınız
- **Türkçe bir kelimeyi desen kipinde aradığınızda yarısı bulunmuyordu.** Düz metin kipinde v1.0.24'te kapatılan eksik desen kipinde ayakta kalmıştı. Aynı belge, aynı sorgu, yalnız "Desen" kutusu farklı: `ç/Ç`, `i/İ`, `ö/Ö`, `ü/Ü` çiftleri katlanmıyor, `ğ/Ğ` ile `ş/Ş` katlanıyordu. `üçgen` sorgusu 3 eşleşmenin 1'ini buluyordu. Üç yol da aynı eksik listeyle çalışıyordu; en kötüsü "Tümünü Değiştir", çünkü belge yarım değişiyor ve etiket yaptığı değişikliği doğru sayıyordu
- **Aynı değişiklik için iki kez sorulması.** Dosya dışarıdan değişince çıkan kutu modal ve dakikalarca açık kalabiliyor. "Diskten Yükle" dediğinizde güncel içerik yükleniyor ama kaydedilen özet diyalog açılmadan önceki hâle ait kalıyordu, yani arabellek diskle birebir aynıyken uygulama onu değişmiş sayıyordu. Kardeş kol ("Kendiminkini Koru") bu dersi zaten biliyordu; asimetri tek koldaydı
- **3664 test** (v1.0.24'te 3632). Bu turun çoğu SyncTeX'te geçti ve yüzey uçtan uca tarandı; düzeltilemeyen tek sınır da kaynağına yazıldı: `lscape` ile çevrilen sayfalarda SyncTeX döndürme öncesi yerleşimi bildiriyor. Her düzeltme yazılmadan önce ölçüldü, yazıldıktan sonra yeniden ölçüldü ve yanındaki gerileme testi, düzeltme bilerek bozulunca düştüğü görüldükten sonra kabul edildi

### v1.0.24: Kuralın Öbür Yarısı

- **Türkçe bir kelime aradığınızda hiçbir şey bulunmuyordu**, hem de iki ayrı yerde. `Ctrl+F` harf duyarsız kipte Türkçe harfleri hiç katlamıyordu: `şekil` sorgusu `Şekil`i, `istanbul` sorgusu `İstanbul`u bulmuyordu; editör ile "Projede Ara" iki ayrı motor kullanıyor ve yalnız biri bunu biliyordu. PDF'in içinde de aynısı: `fontenc` yüklemeyen belgeler aksanlı harfleri iki ayrı glif basıyor, yani `öğrenci` sayfa metninde `¨o˘grenci` olarak duruyor. Kopyalama yolu bunu v1.0.23'ten beri onarıyordu ama arama yolu pdfium'a hâlâ ham sorguyu veriyordu. 58 gerçek PDF, kendi kaynaklarına karşı ölçüldü: kaynakta geçen 642 Türkçe kelimenin 471'i bulunuyordu, artık 523'ü bulunuyor ve daha önce bulunan tek bir kelime bile kaybolmuyor. Vurgu da doğru karakterlere oturuyor, 717 gerçek eşleşmede denetlendi
- **Tablo sihirbazı açtığınız tabloyu yeniden yazıyordu**. Var olan bir `\begin{tabular*}` bloğunu açıp Tamam demek onu `\begin{tabular}`a çeviriyor, genişlik argümanını düşürüyor ve gerçek kolon belirtimini veriymiş gibi ilk hücreye yazıyordu. 39 şablondaki 194 gerçek tablo bloğunda ölçüldü: 8 `tabular*` bloğunun 8'i de böyle bozuluyordu, 10 `tabularx` bloğunun 9'u kendi genişliğini sessizce kaybediyordu (`0.97\textwidth` yerine `\linewidth`). `>{\columncolor{layer7!30}}` gibi iç içe süslü parantez taşıyan bir kolon belirtimi ise hiç okunamıyordu, o da tablonun gövdesine düşüyordu
- **Dışa aktarma arkasında bir şeyler bırakıyordu**. `\graphicspath` yalnızca ana dosyada tek satıra yazıldığında okunuyordu; sınıf dosyasında ya da birkaç satıra yayılmış hâlde bildirilen görsel yolları bulunamıyordu. DOCX çıktısında, şablonun tanımlamadığı bir stile işaret eden tablo var olan bir stile yönlendirilmek yerine SİLİNİYORDU; 16 gerçek şablonda ölçüldü. Şekil başlığı içindeki atıf Markdown çıktısında ham `[@anahtar]` olarak kalıyordu, oysa aynı atıf gövde metninde çözülüyordu
- **Renklendirme kendi dosyanız hakkında yanlış şey söylüyordu**. Komut argümanı içindeki `%` yorum sayılmıyordu; satır içi `\verb|...|` gövdesi de LaTeX kodu gibi renkleniyordu, yani `\verb|$x$|` belgenin geri kalanında matematik rengi açıyor, `\verb|%|` satırın gerisini yoruma çeviriyordu. 133 gerçek şablonda 617 satır içi `\verb` üzerinde ölçüldü: önce 1'i doğru renkleniyordu, sonra 615'i; kalan 2 tanesi gerçek yorum satırının içinde
- **Çökme kurtarması boşuna korkutuyordu**. Ctrl+S'e basıyordunuz, ardından uygulama ölüyordu ve bir sonraki açılışta zaten kaydedilmiş belge için "kaydedilmemiş değişiklik bulundu" diyordu. Satır sonlarını indirgeme kuralı iki yerde ayrı yazılmıştı ve ikisi `\r\r\n` üzerinde ayrışıyordu; o dizi bu uygulamanın eski sürümlerinin diske yazdığı bir hasar. 132 gerçek şablonda ölçüldü: önce 129 yanlış uyarı, şimdi hiç yok; gerçekten kaydedilmemiş içerik ise denenen 396 durumun hepsinde hâlâ yakalanıyor
- **Kuralı duymayan iki yarı daha**. Derleme günlüğü belgenin pdfTeX gerektirdiğini söylediğinde öneri size lualatex'e geçmenizi söylüyordu, yani tam tersini; çünkü motor adını tanıyan desen ile onu karşılığına çeviren tablo ayrı ayrı yazılmıştı. Dosya ağacından derlenen bir belge için verilen kabuk erişimi izni menüden geri alınamıyordu. Yazım denetimi denetlememesi gereken iki bölgeyi denetliyordu ve kırpık bir `.aff` sözlük dosyası hiç onarılmıyordu, yani etkilenen kurulumlarda sözlük hiç yüklenmiyordu
- **İndirdiğiniz dosyalar artık imzalı bir derleme kanıtı taşıyor**, yani bir sürüm dosyasının gerçekten bu depodan ve bu iş akışı koşusundan geldiğini, bu projenin kendisi hakkında söylediklerinden bağımsız olarak doğrulayabiliyorsunuz. Dosyalar hâlâ kod imzalı değil ve SmartScreen ilk çalıştırmada yine uyaracak; doğrulama komutu yukarıdaki indirme bölümünde
- **3632 test** (v1.0.23'te 3586). On üç düzeltme ve neredeyse hepsi aynı biçimde: bir kural ölçülüp öğrenilmiş ve tek bir yere yazılmış, aynı kurala muhtaç olan ikinci yer ondan hiç haberdar olmamış. Bu yüzden bu turda kusurlar değil sınıfın kendisi arandı: kopya sabit listeleri, ayar anahtarlarını, kısayol kayıtlarını ve çeviri kataloğunu birbirine karşı denetleyen betikler yazıldı. O taramaların dördü temiz çıktı ve temiz olarak raporlandı; çıkmayanlar yukarıdaki düzeltmeler. Her düzeltme yazılmadan önce ölçüldü, yazıldıktan sonra yeniden ölçüldü, yanında bir gerileme testiyle geliyor ve her test düzeltme bilerek bozularak düştüğü görüldükten sonra kabul edildi

### v1.0.23: Sizin Yazmadığınız Değişiklik

- **Otomatik kaydetme geldi** ve iki gerçek veri kaybı onunla birlikte kapandı. Kirli sekmeler, yolu olan dosyalar için, seçtiğiniz aralıkta (varsayılan üç dakika) sessizce kaydediliyor; zamanlayıcıdan modal kutu açılmıyor ve derleme tetiklenmiyor. İki kural ölçümle eklendi: diskten silip "Sekmede Tut" dediğiniz dosya diske geri YAZILMIYOR (yoksa sildiğinizi sandığınız dosya dosya ağacında, git durumunda ve derleme çıktısında geri geliyordu), ve dosya dışarıdan değişip "Kendiminkini Koru" dediyseniz otomatik kaydetme o dosyayı ATLIYOR. İkincisi ölçüldü ve ağırdı: dıştan gelen değişiklik (ortak yazar, dal değişimi, senkron istemcisi) üç dakika içinde sessizce yok oluyordu. "Koru" kararı "arabelleğim kalsın" demek, "diski ez" demek değil
- **Silme ve yazma yanlış hedefe gidiyordu**. Klasör değiştirdiğinizde Sürüm Geçmişi listesi eski projenin sürümlerini göstermeye devam ediyordu ve o satırdan "Tüm geçmişi sil" demek YENİ projenin `.git`ini çöp kutusuna yolluyordu; ekranda gördüğünüz geçmiş ise yerinde kalıyordu, onay metni de klasör adını yazmıyordu. DOI ile kaynak eklerken onay kutusunda vazgeçmek diskte boş bir `.bib` bırakıyordu. Kesilen bir yazma kullanıcı sözlüğünü kırpıyordu. "Klasör Aç"ta iptal etmek, önceki sekmenin kaydedilmemiş işaretini düşürmüş oluyordu
- **F2 belgenin BASTIĞI metni değiştiriyordu**. Etiket ya da atıf anahtarını yeniden adlandırmak `verbatim` / `lstlisting` bloğundaki ve satır içi `\verb` içindeki aynı anahtarı da yazıyordu; oysa orası çalışan LaTeX değil, belgenin basılı kod örneği. Otuz dokuz gerçek şablonda ölçüldü, sekiz dosya etkileniyordu: bir tez şablonunda gerçek `\label{fig:Electron}`i değiştirmek örnekteki satırı da değiştiriyor ve kullanıcı çıktının başkalaştığını görmüyordu
- **Dışa aktarma kaynakçasız çıkıyordu**. `.bib` dosyasını bulma iki yerde yazılıydı ve iki uygulama birbirinin düzeltmesini taşımıyordu: dışa aktarma tarafı `\input` zincirini ve sınıf dosyasını taramıyordu. Gerçek pandoc ile uçtan uca ölçüldü, iki tez şablonunda kaynakça hiç bulunmuyordu ve çıktıda yalnız liste eksik değildi, atfın kendisi de boş bir yere basılıyordu ("...PGFPlots ." gibi). Türkçe belgenin `.md` çıktısında kaynakça başlığı İngilizce kalıyordu. Dışa aktarma başarısız olduğunda mesaj, sildiğimiz geçici bir dosyanın adını ve WSL yolunu yazıyordu; artık kendi dosyanızın adını, satır ve sütunla birlikte tek satırda söylüyor
- **Uygulama kendisiyle çelişiyordu**. `@article(anahtar, ...)` biçiminde bir `.bib` girdisi (BibTeX'in ikinci geçerli biçimi) Kaynakça sekmesinde görünüyor ama referans denetimi aynı anahtara "tanımsız atıf" diyor, Alt+tık gitmiyor, F2 `.bib` girdisini atlayıp belgede sarkan atıf bırakıyordu. Aynı sınıftan: tanınmayan `\autocite` gibi sekiz atıf komutu tamamlamada, Alt+tık'ta ve F2'de kaçıyordu; referans denetimi kod örneklerindeki anahtarları gerçek sanıp düzeltilemeyen bulgular basıyordu; kelime sayacı LaTeX sözdizimini ve referans anahtarlarını kelime sayıyordu; Projede Ara kendisiyle örtüşen eşleşmeleri ayrı sonuç sayıp `\\` sorgusunda gerçek şablonlarda 2725 satır gösteriyordu, doğrusu 2629
- **Arayüz önceki belgeyi göstermeyi bıraktı**. Sekme değişince bul çubuğundaki sayaç eski belgenin sayısını gösteriyordu; son sekme kapanınca durum çubuğu kapanan belgenin satır ve kodlama bilgisini tutuyordu; anahat panelinde ham LaTeX komutları ve boş satırlar görünüyordu; kapanmamış tek bir `$` belgenin geri kalanını matematik rengine boyuyordu
- **Sessizce hiçbir şey yapan yollar kapandı**. Sürükleyip bıraktığınız dosya yok sayılıyordu; tablo "Hizala" sekme yokken sessizce dönüyordu; "Dosya Yolunu Kopyala" kaydedilmemiş sekmede boş dönüyordu; PDF "Farklı Kaydet" başarısız olduğunda hiçbir şey söylenmiyordu; yazım denetiminin iki kolu (sözlüğe ekleme ve öneri uygulama) sessizce düşüyordu; derlemeyi siz iptal ettiğinizde log "hata" yazıyordu
- **Küçük ama her gün görünenler**: PDF'ten kopyalanan Türkçe metinde aksanlar ayrık geliyordu (`¨o˘grenci`); PDF arama çubuğunun ikonları tema değişince eski rengi koruyordu; "Farklı Kaydet" düğmesinin etiketi kırpılıp "Farklı Kayd" görünüyordu, ölçtük, düğme 110 piksel ve Türkçe etiket 112 istiyor
- **3586 test** (v1.0.22'de 3082). Otuz düzeltme ve bir yeni özellik; hepsi kodu dosya dosya okumaktan çıktı, hiçbiri bir bildirimden gelmedi. Her biri yazılmadan önce ölçüldü, yazıldıktan sonra yeniden ölçüldü, yanına gerileme testi eklendi ve her test, düzeltme bilerek bozularak gerçekten düştüğü doğrulanarak sınandı. Bu turlarda denenen kusur mutasyonlarının hepsi yakalandı; yanmaması gereken kontrol mutasyonları da yanmadı

### v1.0.22: Bakmadığınız Belge

- **Uygulamanın yanlış belge üzerinde çalıştığı beş yol**, hepsi kapandı, hepsi önce ölçüldü. Yazım düzeltmesi, bulgunun geldiği dosyaya değil o an önünüzde duran dosyaya gidiyordu: A'yı denetleyin, B'ye geçin, paneldeki bulguya sağ tıklayıp bir düzeltme seçin; B değişiyor, A'ya dokunulmuyor ve durum çubuğu başarılı diyordu. Derleme bölümleri diskten okuyor ama yalnız cari sekme kaydediliyordu, yani bir bölümü düzenleyip ana dosyadan derlemek eski metinden bir PDF üretiyor, hata satırları da artık orada olmayan içeriği gösteriyordu. Satıra gitme, hedef açılamadığında imleci cari belgede oynatıyordu. Üç dosya seçip açmak yalnız ilkini açıyordu, sessizce, oysa Linux masaüstü girdisi bir liste bildiriyor. Sürümleme, sürüm adını sormadan önce bütün açık sekmeleri diske yazıyordu; adı iptal etmek dosyalar yazılmış ve sürüm alınmamış hâlde bırakıyordu
- **Geri gelen dosya tek kelime edilmeden eziliyordu**: bir dosya diskten kaybolduğunda uygulama sekmeyi tutmayı öneriyor, kaydedilmemiş iş varken doğru cevap da bu, ama sonra o yolu izlemeyi bırakıyordu. Ölçüldü: dosyayı silin, sekmeyi tutun, bir süre çalışın, dosya bir dal değişiminin geri getirdiği gibi farklı içerikle geri gelsin; hiçbir şey sorulmuyordu ve ilk Ctrl+S geri geleni eziyordu. Artık klasör izleniyor ve geri gelme, diskteki her değişiklik gibi bildiriliyor
- **Farenin tek tekerlek tıkı tablonun bir kolonunu siliyordu**. Qt'nin sayı ve açılır kutuları tekerleğe, yalnızca imlecin altında olmakla yanıt veriyor; sihirbazın ızgarası da tekerlekle kaydırılıyor. Tek tıkla ölçüldü: kolon sayısı beşten dörde düştü ve o hücreler gitti, ortam `tabular`dan `tabularx`a geçti, hizalama ortadan sağa kaydı. Hiçbiri istenmemişti ve bu pencerede geri alma yok. Kutular artık odakta değilken tekerleği yok sayıyor
- **İki çökme ve kabuğa ulaşan bir bağlantı**: yardımcı komut yokken "Klasörde Aç" uygulamayı öldürüyordu, sürümden geri yükleme de dosya yazılamadığında öldürüyordu, çünkü bir Qt slot'unun içindeki yakalanmamış istisna süreci bitiriyor ve öbür sekmelerdeki kaydedilmemiş işi de götürüyor. PDF içindeki bir bağlantı sistem işleyicisine süzgeçsiz gidiyordu, yani indirilen bir belge bir web adresinden başkasını gösterebilirdi; artık yalnız `http`, `https` ve `mailto` açılıyor
- **Derlenmeyen belgeler**: süslü parantez taşıyan bir hücre ve yüzde işareti ya da ters bölü taşıyan bir etiket, LaTeX'in reddettiği bir tablo üretiyordu. Gerçek pdflatex ile ölçüldü: etikette denenen altı karakterin beşi belgeyi derlenemez yapıyor. Eklenen bir görsel derlenmeyen bir belge üretiyordu. Açılması yarıda kesilen bir sözlük kalıcı olarak bozuk kalıyordu; sıkıştırılmış sözlüğü üreten paketleme adımı da kesilince yarım bir dosya bırakabiliyordu
- **PDF görünümü yerinizi koruyor**: yakınlaştırmak okuduğunuz yeri kaybediyordu; iç bağlantı yatay konumu hesaplayıp atıyordu; sayfa büyütülünce arama hedefi ekranın dışında kalıyordu; Ctrl+F zaten açık olan PDF arama çubuğunu kapatıp siliyordu; Ctrl+tık ters aramanın yanına bir de bağlantı tıklıyordu; çift tıklama kelimeyi baştaki boşlukla birlikte seçiyordu
- **Arayüz yerinizi kaybetmeyi bıraktı**: dosya ağacının her yenilenmesi açtığınız klasörleri kapatıp seçimi siliyordu, yani bir klasörün içinde dosya yaratmak az önce yarattığınız dosyayı gizliyordu; dışarıdan gelen her değişiklik de siz çalışırken aynısını yapıyordu. Açık belgenin bağımlılık listesi de aynı anda siliniyordu. Dosya diyalogları hangi projenin açık olduğunu bilmiyordu. Silinen dosya uyarıları üst üste yığılabiliyordu. Bir kez verilen kabuk erişimi izni geri alınamıyordu. Sekme sağ tık menüsünün iki dalı yanlış iş yapıyordu. SyncTeX, araç kurulu değilken "eşleşme bulunamadı" diyordu
- **3082 test** (v1.0.21'de 2563). Otuz altı düzeltme, yine bir bildirimden değil kodu dosya dosya okumaktan çıktı. Her biri yazılmadan önce ölçüldü, yazıldıktan sonra yeniden ölçüldü, her birinin yanında bir gerileme testi var ve her test, düzeltme bilerek bozularak gerçekten düştüğü doğrulanarak sınandı. Kuruluma giren 99 kullanılmayan Qt dil kataloğu da çıkarıldı

### v1.0.21: Satır Satır Denetim ve Belgeyi Bozan On İki Yol
- **Çalışmanızı uyarı vermeden bozan on iki yol**, hepsi kapandı. Yazım önerisi uygulanınca çevresindeki metin bozuluyordu. Eski kodlamayla kaydedilmiş bir belge dışa aktarılırken sessizce bozuluyordu, DOI ile kaynak eklemek de `.bib` dosyasına aynısını yapıyordu. Kaydetmek symlink'i koparıyor, ilk satır uzun olduğunda CRLF satır sonunu düşürüyordu. Yorum kaldırma, yorum olmayan bir yüzde işaretini siliyordu. Bul ve değiştir, size gösterdiği eşleşmeyi değil başkasını değiştiriyordu. Sihirbazın sınırlarını aşan bir tablo satır ve sütun kaybediyordu, üstelik haber vermeden. Yanlış kılıf seçmek bloğu siliyordu. Klasörü yeniden adlandırmak, açık sekmelerini dosyalar silinmiş gibi kapatıyordu. Kaydedilmemiş bir sekme, sizin baktığınız sekmenin üzerine yazıyordu. Açık dosya başka bir sürücüdeyken sürüm eylemi uygulamayı öldürüyordu. Ctrl+V ile görsel yapıştırmak çökertiyordu
- **Yazım denetimi artık gerçekten denetliyor**: sözlük doğrudan hedefine yazılıyordu ve yarıda kesilen bir yazma, hiç onarılmayan kırpılmış bir dosya bırakıyordu, çünkü güncellik ölçütü boyuta değil zaman damgasına bakıyordu. Üçte birine kırpılmış bir sözlükle ölçüldü: günlük on Türkçe kelimenin sekizi yanlış yazılmış sayılıyordu, yani özellik çalışıyor görünüp neredeyse her kelimenin altını çiziyordu. Yazma artık atomik. v1.0.20 ayrıca hiçbir paket denetiminden geçmeden çıkmıştı: yapıyı doğrulayan betik duruyordu ama onu çağıran hiçbir şey yoktu
- **PDF görünümü**: yatay bir sayfadaki iç bağlantı, hedefinin 5864 piksel uzağına götürüyordu. Arama vurgusu sayfanın çizilmesini bekliyor, sonra da yakınlaştırınca kayboluyordu. Sayfa çizilirken fareyi hareket ettirmek arayüzü dondurabiliyordu. Aramayı ya da çizimi durdurmak belgeyi arkada açık bırakıyordu. Sunum modu ekranı doldurmuyor, dosya değişince de kapanmıyordu. Sağ tık menüsü tıkladığınız yerin çok uzağında açılıyordu
- **Türkçe arayüzde İngilizce kalan menüler**: editörün sağ tık menüsü QScintilla'nın kendi kataloğundan, süzme kutularınınki Qt'nin kataloğundan besleniyor ve ikisi de hiç yüklenmiyordu. Artık ikisi de arayüz dilinde
- **Verbatim blokları yorum gibi görünüyordu**: renk sönük griden ödünç alınıyordu. Yorum renginden uzaklığı gruvbox'ta 3, monokai'de 63, dracula'da 64 ölçüldü; bu projenin gözün ayırt edebildiği sınır olarak zaten 40'ı kullandığı ölçekte. Artık her tema ham kod için kendi rengini taşıyor, `\begin` ve `\end` de komut olarak renkleniyor, böylece bloğun nerede başlayıp bittiği görünüyor
- **Derleme**: seçenekli `\usepackage[..]{fontspec}` hiç görülmüyordu, motor yanlış seçiliyordu. Kabuk erişimi izni derlenen belgeye değil proje köküne bağlıydı. "Tanımsız komut" ipucu, düşen komutu değil sağlam olanı gösteriyordu, çünkü günlük bağlam satırını tam suçlunun ardından kesiyor. Saran uyarılar yarım ayrıştırılıyor, hatalar yanlış dosyaya atfediliyordu
- **Gezinme**: Ctrl+P'de dosya adını yazıp Enter'a basmak başka bir dosya açıyordu. Büyük harfli `.TEX` uzantılı dosyalar iki referans taramasında da atlanıyordu. Proje araması, sonuç listesi tam sınıra değdiğinde eksiksiz listeyi "kırpıldı" diye gösteriyordu
- **Pencere küçük ekranlara sığmıyordu** ve uygulama, sistem dili ne olursa olsun dünyanın her yerinde Türkçe açılıyordu
- **Anahat ve kelime sayımı kodu düz metin sanıyordu**: verbatim bloğunun içindeki örnek bölümler anahatta listeleniyor, kod bloğundaki bir `\end{document}` kelime sayımını yarıda kesiyordu
- **2563 test** (v1.0.20'de 1874). Elli beş düzeltme, neredeyse hepsi bir bildirimden değil kodu dosya dosya okumaktan çıktı. Her birinin yanında bir gerileme testi var ve her test, düzeltme bilerek bozularak gerçekten düştüğü doğrulanarak sınandı
